...
def run_gvecMin(file_path):
    ...
    for i in range(len(H)):
        ...
        Z1[y_idx, x_idx] = (gvecMin[i])
        Z1_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])
    ...
    for i in range(len(H)):
        ...
        Z2[y_idx, x_idx] = (gvecMin[i])
        Z2_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])
    ...
    for i in range(len(H)):
        ...
        Z3_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])
    ...
    for i in range(len(alpha1)):
        ...
        y_idx = np.where(np.unique(beta2) == beta2[i])[0]
        Z4[y_idx, x_idx] = (gvecMin[i])
        Z4_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])
    ...
    for i in range(len(alpha1)):
        x_idx = np.where(np.unique(alpha1) == alpha1[i])[0]
        y_idx = np.where(np.unique(beta1) == beta1[i])[0]
        Z5[y_idx, x_idx] = (gvecMin[i])
        Z5_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])
    ...
    for i in range(len(beta2)):
        ...
        y_idx = np.where(np.unique(beta1) == beta1[i])[0]
        Z6[y_idx, x_idx] = (gvecMin[i])
        Z6_r[y_idx, x_idx] = (gvecMin_r - gvecMin[i])


    fig, axes = plt.subplots(3, 3, figsize=(9, 9))

    cax1 = axes[0, 0].pcolormesh(X1, Y1, Z1_r, cmap='seismic', vmin=-100, vmax=100, shading='auto')
    axes[0, 0].set_xlabel('H')
    axes[0, 0].set_ylabel('alpha1')
    cbar1 = fig.colorbar(cax1, ax=axes[0, 0], shrink=0.5)
    cbar1.set_label('gvecMin_residual (Hz)')

    cax2 = axes[1, 0].pcolormesh(X2, Y2, Z2_r,  cmap='seismic', vmin=-100, vmax=100, shading='auto')
    axes[1, 0].set_xlabel('H')
    axes[1, 0].set_ylabel('beta2')
    cbar2 = fig.colorbar(cax2, ax=axes[1, 0], shrink=0.5)
    cbar2.set_label('gvecMin_residual (Hz)')

    cax3 = axes[2, 0].pcolormesh(X3, Y3, Z3_r,  cmap='seismic', vmin=-100, vmax=100, shading='auto')
    ...
    cax4 = axes[1, 1].pcolormesh(X4, Y4, Z4_r,  cmap='seismic', vmin=-100, vmax=100, shading='auto')
    axes[1, 1].set_xlabel('alpha1')
    axes[1, 1].set_ylabel('beta2')
    cbar4 = fig.colorbar(cax4, ax=axes[1, 1], shrink=0.5)
    cbar4.set_label('gvecMin_residual (Hz)')

    cax5 = axes[2, 1].pcolormesh(X5, Y5, Z5_r,  cmap='seismic', vmin=-100, vmax=100, shading='auto')
    axes[2, 1].set_xlabel('alpha1')
    axes[2, 1].set_ylabel('beta1')
    cbar5 = fig.colorbar(cax5, ax=axes[2, 1], shrink=0.5)
    cbar5.set_label('gvecMin_residual (Hz)')

    cax6 = axes[2, 2].pcolormesh(X6, Y6, Z6_r,  cmap='seismic', vmin=-100, vmax=100, shading='auto')
    axes[2, 2].set_xlabel('beta2')
    axes[2, 2].set_ylabel('beta1')
    cbar6 = fig.colorbar(cax6, ax=axes[2, 2], shrink=0.5)
    cbar6.set_label('gvecMin_residual (Hz)')

    axes[0, 1].remove()
    axes[0, 2].remove()
    axes[1, 2].remove()

    fig.suptitle('minimum group velocity residual', fontsize=16)
